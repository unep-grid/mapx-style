...
Usage:
  uv run skills/build_glyphs.py
  uv run skills/build_glyphs.py --no-upload            # generate only, skip S3
  uv run skills/build_glyphs.py --env prod staging     # upload to both envs
  uv run skills/build_glyphs.py --env all              # upload to all envs
  uv run skills/build_glyphs.py --out-dir /mnt/large_volume/tmp/glyphs
...
S3 key:         style/{env}/assets/glyphs/<MapLibre font name>/{0-255}.pbf
...
def _upload_and_set_public(pbf: Path, out_dir: Path, env: str) -> str:
    from set_acl import set_public_acl

    s3, bucket = _get_client()
    relative = pbf.relative_to(out_dir)
    s3_key = f"style/{env}/assets/glyphs/{relative}"
...
def main() -> None:
    ...
    for env in envs:
        console.print(f"\nUploading to [cyan]{env}[/cyan] with [cyan]{args.workers}[/cyan] workers…")
        ...
        with Progress(
            ...
            task = progress.add_task(f"Uploading PBFs [{env}]…", total=len(pbf_files))

            with ThreadPoolExecutor(max_workers=args.workers) as pool:
                futures = {pool.submit(_upload_and_set_public, f, out_dir, env): f for f in pbf_files}
        ...
        console.print(
            ...
            f"style/{env}/assets/glyphs/[/green]"
        )
